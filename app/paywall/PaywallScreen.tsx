...
const PaywallScreen: React.FC<PaywallScreenProps> = () => {
    ...
    useEffect(() => {
        const animateElements = () => {
            ...
            buttonScale.value = withDelay(320, withTiming(1, { ...animationConfig, duration: 500 }));
    ...
    const handlePurchase = async (packageToPurchase: PurchasesPackage) => {
        try {
            ...
            const hasActiveProduct = purchaseResult.customerInfo.activeSubscriptions.length > 0;
            ...
            >
                ...
                >
                    ...
                    return (
                        ...
                        >
                            <View style={{alignItems: 'center', justifyContent: 'center', padding:8, backgroundColor:'rgba(255, 252, 247, 0.66)', borderRadius:20}}>
    ...
    const getLocalizedPackageInfo = (pkg: PurchasesPackage, index: number, isReferral: boolean = false) => {
        ...
        if( price.includes("3.9")){
            ...
            description = isReferral ? t('paywall.annualReferralDescription') : t('paywall.annualDescription');
            ...
            if (isReferral) {
                ...
                const discounted = numericPrice * 0.70; // 30% descuento
                discountedPrice = `$${discounted.toFixed(2)}`;
            }
        } else if (isMonthly ) {
            description = isReferral ? t('paywall.monthlyReferralDescription') : t('paywall.monthlyDescription');
            ...
            if (isReferral) {
                ...
                const numericPrice = parseFloat(originalPrice.replace(/[^0-9.]/g, ''));
                const discounted = numericPrice * 0.50; // 50% descuento
                discountedPrice = `$${discounted.toFixed(2)}`;
            }
        } else {
        ...
        return { title, description, price, pricePeriod, originalPrice, discountedPrice };
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={{ flex:1, flexDirection:"row", alignItems: "center", justifyContent: "space-between", position:"absolute", top:0, left:0, right:0, zIndex:10, paddingHorizontal:20 }}>
                <TouchableOpacity onPress={()=> {
                    if( fromIntro === "true") router.replace("/home");
                    else router.back();
                }} style={{alignItems: 'center'}}>
                    <Text style={{ color: '#999999ff', fontSize: 18 }}>{t('common.x')}</Text>
                </TouchableOpacity>
                ...
                <ScrollView contentContainerStyle={styles.scrollContent} style={styles.scrollView}>
                    ...
                    <View style={{marginTop: 0, paddingTop: 0}}>
                        ...
                        <View style={{marginTop:-80, backgroundColor:"#fff"}}>
                            <Reanimated.View style={titleAnimatedStyle}>
                                <Text style={styles.title}>{t('paywall.text1')} <Text style={[styles.title,{color:Colors.primary}]}>{t('common.pro')}</Text> {t('paywall.text1_end')}</Text>
                            </Reanimated.View>
                        ...
                        <Reanimated.View style={[{marginHorizontal:"auto", marginBottom:20}, featuresAnimatedStyle]}>
                        {
                            description.map((desc, idx) => (
                                <View key={idx} style={{ flexDirection: "row", marginTop: 10, paddingHorizontal: 10, alignItems:  "center", maxWidth:"90%" }}>
                                    {
                                        (idx === 0)? (
                                            <View style={[styles.iconDetails,{backgroundColor:Colors.primary}]}>
                                                <FontAwesome name="clock-o" size={16} color="white"  />
                                            </View>
                                        ):(idx === 1) ? (
                                            <View style={[styles.iconDetails,{backgroundColor:"#2196F3"}]}>
                                                <FontAwesome name="language" size={16} color="white"/>
                                            </View>
                                        ):(idx === 2) ? (
                                            <View style={[styles.iconDetails,{backgroundColor:"#4CAF50"}]}>
                                                <FontAwesome name="phone" size={16} color="white"/>
                                            </View>
                                        ): (
                                            <View style={[styles.iconDetails,{backgroundColor:"#9C27B0"}]}>
                    ...
                    <View style={styles.packagesContainer}>
                        ...
                        {allPackages.map((pkg: PurchasesPackage, index: number) => {
                            ...
                            const isReferral = pkg.identifier.toLowerCase().includes('referral_code');
                            const { title, description, price, pricePeriod, originalPrice, discountedPrice } = getLocalizedPackageInfo(pkg, index, isReferral);
                            ...
                            return (
                                <Reanimated.View key={pkg.identifier} style={isSelected ? packageBounceStyle : {}}>
                                    <TouchableOpacity
                                    style={[
                                        ...
                                        <View style={{flexDirection: 'column', alignItems: 'flex-end'}}>
                                            {isReferral && originalPrice && discountedPrice ? (
                                                <>
                                                    ...
                                                    <Text style={{fontWeight: 500, textAlign: 'center', fontSize: 12}}>{pricePeriod}</Text>
                                                </>
                                            ) : (
                                                <>
