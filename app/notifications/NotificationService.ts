...
class NotificationService {
    ...
    if (Device.isDevice) {
      ...
      try {
        const pushTokenData = await Notifications.getExpoPushTokenAsync({
          projectId: Constants.expoConfig?.extra?.eas?.projectId || '70e433ab-14f4-4bbc-8070-e2adf248202d',
        });
  ...
  private handleNotificationResponse(response: Notifications.NotificationResponse): void {
    ...
    // You can add specific navigation based on notification type here
    if (data?.screen) {
  ...
  async sendTokenToServer(): Promise<boolean> {
    try {
      ...
      // Usar apiClient que automáticamente agrega company_id como query param
      const response = await apiClient.post('/notifications/register-device/', deviceInfo);
      
      console.log('✅ Device token registered successfully with company:', response.data);
      return true;
    } catch (error) {
  ...
  // Método para registrar token en el servidor Django
  private async registerTokenOnServer(deviceInfo: any): Promise<boolean> {
    try {
      return await registerDeviceTokenWithDjango(deviceInfo.token);
    } catch (error) {
      console.error('Error registering token with Django server:', error);
      return false;
    }
  }
  ...
  private async getDeviceId(): Promise<string> {
    try {
      ...
      if (!deviceId) {
        ...
        const deviceInfo = Device.osBuildId || Device.osInternalBuildId || Date.now().toString();
        deviceId = `${Platform.OS}_${deviceInfo}_${Math.random().toString(36).substring(2)}`;
